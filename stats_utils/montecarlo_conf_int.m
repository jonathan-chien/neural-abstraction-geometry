function confInt = montecarlo_conf_int(distr,varargin)
% Calculate confidence interval(s) of specified size around mean of
% supplied draws from distribution.
%
% Example syntaxes (see PARAMETERS for arg info):
% -----------------
% confInt = montecarlo_conf_int(distr)
% confInt = montecarlo_conf_int(distr, interval);
% confInt = montecarlo_conf_int(distr, interval, dim);
% confInt = montecarlo_conf_int(distr, [], dim);
% 
% PARAMETERS
% ----------
...
%             return a 2-vector corresponding to the confidence interval
...
%             distributions, and confidence interval bounds will be
...
% RETURNS
% -------
% confInt -- If distr was passed in as a vector, confInt is a 2-vector
%            whose first and second elements are, respectively, the lower
%            and upper bound of the confidence interval of specified size.
%            If distr was passed in as an m x n or n x m matrix (see distr
%            under PARAMETERS), confInt is an m x 2 or 2 x m, matrix where
%            the i_th row or column (of length 2), respectively, correspond
%            to the lower and upper bound of the i_th distribution, for i =
%            1:m.
%
...
if any(size(distr)==1) 
    confInt = prctile(distr, [50-interval/2 50+interval/2]);
elseif all(size(distr)>1)
    confInt = prctile(distr, [50-interval/2 50+interval/2], dim);
end
